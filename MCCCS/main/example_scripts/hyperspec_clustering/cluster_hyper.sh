...
do
	if  [ "$dir" = "CVS" ]; then
		...
		$WEKA weka.filters.unsupervised.instance.Resample -S 1 -Z 0.05 < "${dir}/${dir}_7.arff" > "${dir}/${dir}_resampled.arff"
		#train clustering on resampled data, ignoring class attribute
		$WEKA weka.clusterers.EM -I 10 -N $CLASSCOUNT -t "${dir}/${dir}_resampled.arff" -T "${dir}/${dir}_7.arff" -p 0 > "${dir}/${dir}_predict.arff"
...
#$WEKA weka.filters.unsupervised.attribute.AddCluster -i "${dir}/${dir}.arff" -serialized fgbg.model -o "${dir}/result.arff" -c last
		$ADDCLU "${dir}/${dir}_7.arff" "${dir}/${dir}_predict.arff" "${dir}/result.arff"
		echo -n "."
