if [ "$#" -ne 3 ]; then
    echo "Please supply the path to the mcccs.jar as parameter 1, the path to the data-files as parameter 2 and the number of disired cluster-classes!"
	exit 1
fi
...
if [ "$(uname)" == "Darwin" ]; then
	export JAVA="java -Xmx4g -Dapple.awt.UIElement=false -Djava.awt.headless=true -cp $MBP workflow"
else
	export JAVA="java -classpath $MBP workflow"
fi 
