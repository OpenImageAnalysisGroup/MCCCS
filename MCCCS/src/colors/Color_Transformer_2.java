...
public class Color_Transformer_2 implements PlugInFilter {
	...
	public boolean showDialog() {
		...
		String[] RGBcolourspaces = { "sRGB", "Adobe RGB", "ProPhoto RGB",
				"eciRGB v2", "Custom" };
		
		String[] colourspaces = { "XYZ", "Yxy", "YUV", "YIQ", "YCbCr", "Luv",
				"Lab", "AC1C2", "I1I2I3", "Yuv", "YQ1Q2", "HSI", "HSV/HSB",
				"HSL", "HCY", "LCHLuv", "LSHLuv", "LCHLab", "LSHLab", "CMYK",
				"CMYK plates" };
	...
	public void run(ImageProcessor ip) {
																			...
																			if (colourspace.equals("LCHLab")) {
																				...
																				if (colourspace.equals("LSHLab")) {
																					...
																					if (colourspace.equals("CMYK")) {
																						...
																						if (colourspace.equals("CMYK plates")) {
																							n1 = "cyan plate";
																							n2 = "magenta plate";
																							n3 = "yellow plate";
																							n4 = "black plate";
		...
		if (c1.length == size) {
			...
			ImagePlus imc1 = NewImage.createFloatImage(n1, width, height, 1,
					NewImage.FILL_BLACK);
			ImageProcessor ipc1 = imc1.getProcessor();
			ipc1.setPixels(c1);
			sstack.addSlice(n1, ipc1);
			
			ImagePlus imc2 = NewImage.createFloatImage(n2, width, height, 1,
					NewImage.FILL_BLACK);
			ImageProcessor ipc2 = imc2.getProcessor();
			ipc2.setPixels(c2);
			sstack.addSlice(n2, ipc2);
			
			ImagePlus imc3 = NewImage.createFloatImage(n3, width, height, 1,
					NewImage.FILL_BLACK);
			...
			if (colourspace.equals("CMYK")) {
				ImagePlus imc4 = NewImage.createFloatImage(n4, width, height,
		...
		ImagePlus imluv = new ImagePlus(title + " (" + colourspace + ")",
				sstack);
		imluv.show();
		IJ.resetMinAndMax();
	...
	public void getXYZ() {
		...
		if (RGBcolourspace.startsWith("sRGB")) {
			...
			matrix = new float[][] { { 0.4124564f, 0.3575761f, 0.1804375f },
			...
			if (RGBcolourspace.startsWith("Adobe RGB")) {
				encoding = "gamma22";
				
				matrix = new float[][] { { 0.5767309f, 0.1855540f, 0.1881852f },
				...
				if (RGBcolourspace.startsWith("ProPhoto RGB")) {
					encoding = "gamma18";
					
					matrix = new float[][] { { 0.7976749f, 0.1351917f, 0.0313534f },
					...
					if (RGBcolourspace.startsWith("eciRGB v2")) {
						encoding = "Lstar";
						
						matrix = new float[][] { { 0.6502043f, 0.1780774f, 0.1359384f },
						...
						if (RGBcolourspace.startsWith("Custom")) { // ProPhoto 2.2 5500
							encoding = "gamma22";
							
							matrix = new float[][] {
									{ 0.7857983f, 0.1360043f, 0.0350196f }, // ProPhoto with a
									// D55 white
									// reference
									{ 0.2837516f, 0.7161527f, 0.0000957f },
									{ 0.0000000f, 0.0000000f, 0.9217041f } };
		...
		for (int q = 0; q < size; q++) {
			...
			if (encoding.equals("sRGB")) {
				rf[q] = (rf[q] > 0.04045f) ? (new Double(Math.exp(Math
						.log((rf[q] + 0.055) / 1.055) * 2.4))).floatValue()
						: rf[q] / 12.92f;
				gf[q] = (gf[q] > 0.04045f) ? (new Double(Math.exp(Math
						.log((gf[q] + 0.055) / 1.055) * 2.4))).floatValue()
						: gf[q] / 12.92f;
				bf[q] = (bf[q] > 0.04045f) ? (new Double(Math.exp(Math
						.log((bf[q] + 0.055) / 1.055) * 2.4))).floatValue()
						: bf[q] / 12.92f;
			}
			
			else
				if (encoding.equals("gamma22")) {
					rf[q] = new Double(Math.exp(Math.log(rf[q]) * 2.2))
							.floatValue();
					gf[q] = new Double(Math.exp(Math.log(gf[q]) * 2.2))
							.floatValue();
					bf[q] = new Double(Math.exp(Math.log(bf[q]) * 2.2))
							.floatValue();
				}
				
				else
					if (encoding.equals("gamma18")) {
						rf[q] = new Double(Math.exp(Math.log(rf[q]) * 1.8))
								.floatValue();
						gf[q] = new Double(Math.exp(Math.log(gf[q]) * 1.8))
								.floatValue();
						bf[q] = new Double(Math.exp(Math.log(bf[q]) * 1.8))
								.floatValue();
					}
					
					else
						if (encoding.equals("Lstar")) {
							rf[q] = (rf[q] > 0.08f) ? (new Double(Math.exp(Math
									.log((rf[q] + 0.16) / 1.16) * 3))).floatValue()
									: rf[q] / 9.033f;
							gf[q] = (gf[q] > 0.08f) ? (new Double(Math.exp(Math
									.log((gf[q] + 0.16) / 1.16) * 3))).floatValue()
									: gf[q] / 9.033f;
							bf[q] = (bf[q] > 0.08f) ? (new Double(Math.exp(Math
									.log((bf[q] + 0.16) / 1.16) * 3))).floatValue()
									: bf[q] / 9.033f;
						}
			
			X = matrix[0][0] * rf[q] + matrix[0][1] * gf[q] + matrix[0][2]
					* bf[q];
			Y = matrix[1][0] * rf[q] + matrix[1][1] * gf[q] + matrix[1][2]
					* bf[q];
			Z = matrix[2][0] * rf[q] + matrix[2][1] * gf[q] + matrix[2][2]
					* bf[q];
		 ...
		 * { // ColorMatch RGB encoding = "gamma18";
		 * matrix = new float[][] {{0.5093439f, 0.3209071f, 0.1339691f},
		 * {0.2748840f, 0.6581315f, 0.0669845f}, {0.0242545f, 0.1087821f,
		 * 0.6921735f}};
		 * Lwhite = D50; }
		 * else if (RGBcolourspace.startsWith("Custom")) { // Wide Gamut RGB
		 * encoding = "gamma22";
		 * matrix = new float[][] {{0.7161046f, 0.1009296f, 0.1471858f},
		 * {0.2581874f, 0.7249378f, 0.0168748f}, {0.0000000f, 0.0517813f,
		 * 0.7734287f}};
		 * Lwhite = D50; }
		 * else if (RGBcolourspace.startsWith("Custom")) { // Beta RGB encoding
		 * = "gamma22";
		 * matrix = new float[][] {{0.6712537f, 0.1745834f, 0.1183829f},
		 * {0.3032726f, 0.6637861f, 0.0329413f}, {0.0000000f, 0.0407010f,
		 * 0.7845090f}};
		 * Lwhite = D50; }
		 * else if (RGBcolourspace.startsWith("Custom")) { // Russell RGB
		 * encoding = "gamma22";
		 * matrix = new float[][] {{0.7015731f, 0.1554157f, 0.0998333f},
		 * {0.3151995f, 0.6648338f, 0.0199667f}, {0.0000000f, 0.0431710f,
		 * 0.8785331f}};
		 * Lwhite = D55; }
		 * else if (RGBcolourspace.startsWith("Custom")) { // Melissa RGB
		 * encoding = "sRGB";
		 * matrix = new float[][] {{0.7976749f, 0.1351917f, 0.0313534f},
	...
	public void getYxy() {
		...
		for (int q = 0; q < size; q++) {
			...
			float xx = (X == 0f && Y == 0f && Z == 0f) ? 0f
					: (X / ((X + Y + Z)));
			float yy = (X == 0f && Y == 0f && Z == 0f) ? 0f
					: (Y / ((X + Y + Z)));
	...
	public void getLuv() {
		...
		for (int q = 0; q < size; q++) {
			...
			float up = (X == 0f && Y == 0f && Z == 0f) ? 0f : (4f * X / ((X
					+ 15f * Y + 3f * Z)));
			float vp = (X == 0f && Y == 0f && Z == 0f) ? 0f : (9f * Y / ((X
					+ 15f * Y + 3f * Z)));
	...
	public void getYuv() {
		...
		for (int q = 0; q < size; q++) {
			...
			if (Y != 0f) {
				u = 3f * (rf[q] - bf[q]) / 2f * Y;
				v = (new Double(Math.sqrt(3.0)).floatValue())
						* (2f * gf[q] - rf[q] - bf[q]) / 2f * Y;
			} else {
				u = 0f;
				v = 0f;
			}
	...
	public void getHSI() {
		...
		for (int q = 0; q < size; q++) {
			...
			if (del_Max == 0f) { // This is a gray, no chroma...
				...
				float del_R = (((var_Max - rf[q]) / 6f) + (del_Max / 2f))
						/ del_Max;
				float del_G = (((var_Max - gf[q]) / 6f) + (del_Max / 2f))
						/ del_Max;
				float del_B = (((var_Max - bf[q]) / 6f) + (del_Max / 2f))
						/ del_Max;
	...
	public void getLCHLuv() {
		...
		for (int q = 0; q < size; q++) {
			...
			H = new Double((Math.atan2(v, u) + (2f * Math.PI)) % (2f * Math.PI))
					.floatValue(); // angle from 0 to 2 pi
	...
	public void getLSHLuv() {
		...
		for (int q = 0; q < size; q++) {
			...
			L = l;
			S = (new Double(13 * Math.sqrt(((u - up) * (u - up))
					+ ((v - vp) * (v - vp))))).floatValue();
	...
	public void getLCHLab() {
		...
		for (int q = 0; q < size; q++) {
			...
			H = new Double((Math.atan2(b, a) + (2f * Math.PI)) % (2f * Math.PI))
					.floatValue(); // angle from 0 to 2 pi
			H = new Double((H / (2f * Math.PI)) * 100f).floatValue(); // normalize
			// to
			// 0-100
	...
	public void getCMYKplates() {
		...
		for (int q = 0; q < size; q++) {
			...
			if (D > Kstart)
				fD = Klimit
						* (new Double(Math.exp(Math.log((D - Kstart)
								/ (1 - Kstart))
								* g))).floatValue();
		...
		ImagePlus imc1 = NewImage.createRGBImage(n1, width, height, 1,
				NewImage.FILL_BLACK);
		ImageProcessor ipc1 = imc1.getProcessor();
		ipc1.setPixels(d1);
		sstack.addSlice(n1, ipc1);
		
		ImagePlus imc2 = NewImage.createRGBImage(n2, width, height, 1,
				NewImage.FILL_BLACK);
		ImageProcessor ipc2 = imc2.getProcessor();
		ipc2.setPixels(d2);
		sstack.addSlice(n2, ipc2);
		
		ImagePlus imc3 = NewImage.createRGBImage(n3, width, height, 1,
				NewImage.FILL_BLACK);
		ImageProcessor ipc3 = imc3.getProcessor();
		ipc3.setPixels(d3);
		sstack.addSlice(n3, ipc3);
		
		ImagePlus imc4 = NewImage.createRGBImage(n4, width, height, 1,
				NewImage.FILL_BLACK);
		ImageProcessor ipc4 = imc4.getProcessor();
		ipc4.setPixels(d4);
		sstack.addSlice(n4, ipc4);
	...
	void error() {
		IJ.showMessage("Colour_transform",
				"This plugin converts an RGB image into \n"
