...
if [ "$(uname)" == "Darwin" ]; then
	export JAVA="/mnt/16TB_1/jdk1.8.0_45/jre/bin/java -Xmx50g -Dapple.awt.UIElement=true -cp $MBP workflow"
else
	export JAVA="/mnt/16TB_1/jdk1.8.0_45/jre/bin/java -Xmx50g -cp $MBP workflow"
fi 
...
export WEKA="/mnt/16TB_1/jdk1.8.0_45/jre/bin/java -Xmx25g -cp $WEKAJAR"
...
then
	...
	then
		...
		then
			NPROCS=5
			echo "Operation Mode: utilizing 4 CPUs ($NPROCS)"
		else
...
par="/opt/Bio/parallel/20160322/bin/parallel --gnu --tmpdir /mnt/16TB_1/MacroBot_Training/tmp/ --gnu -j $NPROCS"
